        ...
        <div class="widget has-shadow">
            <div class="widget-body">
                <div class="table-responsive">
                    <table class="table table-hover mb-0 " id="pending_payments_tbl">
                        <thead>
                            <tr>
                                <th>Invoice No</th>
                                <th>Customer Name </th>
                                ...
                                <th>Total Amount</th>
                                <th data-sorter="false">Pending Amount</th>
                                <th class="sorter-shortDate dateFormat-ddmmyyyy">Invoice Date</th>
                        ...
                        <tbody>
                            @isset($history)
                                @foreach ($history as $item)
                                    @if (($item->grand_total-$item->total_amount)!=0)
                                    <tr class="{!! \Carbon\Carbon::createFromFormat('d/m/Y', $item->invoice_date)->format('Y-m-d')<=now()->subDays(30)?'table-danger':'' !!}">
                                        <td>{!! $item->invoice_no !!}</td>
                                        <td>{!! $item->customer['full_name'] !!}</td>
                                        <td>{!! $item->paymentReceiver->fullName !!}</td>
                                        <td>{!! $item->grand_total !!}</td>
                                        <td>{!! $item->total_amount?($item->grand_total-$item->total_amount):$item->grand_total !!}</td>
...
<script src="{{ asset('assets/js/datepicker/daterangepicker.js') }}"></script>
    <script>
        $(document).ready(function(){
            $('#pending_payments_tbl').tablesorter({
                cssAsc: 'up',
		        cssDesc: 'down',
                cssNone: 'both'
            });
